# Portfolio product and design contract

## Purpose

This is Paranjay's personal portfolio before it is a reusable template. Every visible item must be either real, clearly labelled as WIP/example, or absent.
...
## Completion checklist for a change

1. Address every related browser annotation, not just the most recent one.
2. Update `PORTFOLIO_NEXT_STEPS.md` for deferred work and required user input.
3. Run type checking and targeted linting; do not run local production builds on this machine.
4. Commit and push each completed pass to `main` so Vercel deploys it.

## Open design work

- Replace the experimental KP cover with a bespoke mark that has the polish of the original isometric composition.
